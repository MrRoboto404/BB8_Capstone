...
K_xy = gain_mod_t.*get_gains_LQR_xy
K_vert(1) = gain_mod_1 * K_vert(1);
K_vert(2) = gain_mod_2 * K_vert(2);
K_vert(3) = gain_mod_3 * K_vert(3);
K_vert(4) = gain_mod_4 * K_vert(4);
K_vert
% K_vert = [0, -110, -2.5, -13.5]
...
ic_xz = [0; deg2rad(5); 0; deg2rad(0)];  % initial condition deg
ic_yz = [0; deg2rad(5); 0; 0];
ic_xy = [0; 0];
T_sim = 5; % s
% %%
% T_max = 5000;
...
subplot(3,3,7)
plot(t,rad2deg(dpx),t,rad2deg(dtx))
ylabel('Angle rate (deg/s)')
legend('dotphi x','dottheta x')
grid

subplot(3,3,8)
plot(t,rad2deg(dpy),t,rad2deg(dty))
ylabel('Angle rate (deg/s)')
legend('dotphi y','dottheta y')
grid
...
sgtitle(graph_title(2));
subplot(3,1,1)
plot(t,T1*27,t,T2*27,'g',t,T3*27,'r--')
legend('T_1','T_2','T_3')
xlabel('Time (s)')
ylabel('Motor Torque (Nm)')
title('Motor spec');
...
%%
Tx_sim = -(cos(params().alpha)*(T1*cos(params().beta)) ...
    - T2*sin(params().beta + pi/6) ...
    + T3*sin(params().beta - pi/6));
Ty_sim = cos(params().alpha)*(-T1*sin(params().beta)) ...
- T2*cos(params().beta + pi/6) + T3*cos(params().beta - pi/6);
%%
data = readtable('good-115-25-5.csv');
...
figure
subplot(2,1,1)
plot(data.Time_us(1:400), rad2deg(data.Roll(1:400)),t(1:204),rad2deg(ty(1:204)))
legend("Roll data (deg)","Roll sim (deg)")
grid;
subplot(2,1,2)
plot(data.Time_us(1:400), Ty(1:400),t(1:204), -Ty_sim(1:204))
legend("Ty data (Nm)", "Ty sim (Nm)")
grid;
%%
% figure
% subplot(7,1,1)
