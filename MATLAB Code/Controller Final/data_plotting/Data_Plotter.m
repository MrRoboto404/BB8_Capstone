clc; clear;
close all;
data = readtable('158993008_motor_data.csv');
...
Tx = -(cos(params().alpha)*(data.motor_torq_1*cos(params().beta)) ...
    - data.motor_torq_2*sin(params().beta + pi/6) ...
    + data.motor_torq_3*sin(params().beta - pi/6));

Ty = cos(params().alpha)*(-data.motor_torq_1*sin(params().beta)) ...
- data.motor_torq_2*cos(params().beta + pi/6) + data.motor_torq_3*cos(params().beta - pi/6);
...
subplot(7,1,6)
plot(data.Time_us, data.motor_torq_1,data.Time_us, data.motor_torq_2,data.Time_us, data.motor_torq_3)
subplot(7,1,7)
...
%%
figure
plot(data.Time_us(1:200), data.T1(1:200), data.Time_us(1:200), data.motor_torq_1(1:200))
