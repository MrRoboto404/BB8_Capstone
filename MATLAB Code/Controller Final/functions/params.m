...
p.m_ball = 2.9413;   %kg, Mass of the Ball
p.m_body = 11.6;    %kg, Mass of the body
p.m_omni = 0.119; %kg, reverse engineered from inertia given

%_______Lengths_______
p.r_ball = .258;   %m, Radius of the ball
...
p.l = 0.232;     %m, Height of the center of gravity of body from center of ball
...
p.i_Gear = sqrt(26.8);  % -, Gear ratio from most recent motor (57BYA74-24-01-PG27)
p.K_T = 0.04813; %Nm/A, motor torque constant from """""

%_______Inertia Constants_______
p.Theta_motor_rotor = 6.33 * 10^-5; % rotor intertia (real small)
...
%           Vertical Planes
p.Theta_body_vertical = 0.25*p.m_body*(p.r_body^2) + (1/12)*p.m_body*(p.h_body^2) + p.m_body*p.l^2;      % kg*m^2, Inertia of the body in vertical planes
